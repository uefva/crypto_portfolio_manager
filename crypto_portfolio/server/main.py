"""HTTP server, price collection, and portfolio API implementation.

This module owns the service runtime. Thin modules in ``crypto_portfolio.server``
re-export its main classes so callers can import by responsibility while we keep
the existing API behavior unchanged during the first structural split.
"""
